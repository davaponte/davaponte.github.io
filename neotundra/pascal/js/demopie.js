...
rtl.module("program",["System","JS","Web","SysUtils","ChartJS"],function () {
  "use strict";
  var $mod = this;
  this.config = null;
  this.dataset = null;
  $mod.$main = function () {
    Chart.defaults.global.tooltips.custom = function (tooltip) {
      ...
      if (pas.System.Assigned(tooltip.yAlign)) {
        tooltipEl.classList.add(tooltip.yAlign)}
       else tooltipEl.classList.add("no-transform");
      if (rtl.length(tooltip.body) > 0) {
        if (tooltip.title != null) {
          titleLines = tooltip.title}
         else titleLines = new Array();
        bodyLines = tooltip.body.map(function (bodyItem, index, arr) {
          var Result = undefined;
          Result = rtl.getObject(bodyItem).lines;
          return Result;
        });
        innerHtml = "<thead>";
        titleLines.forEach(function (title, index, arr) {
          var Result = false;
        ...
        bodyLines.forEach(function (body, index, arr) {
          var Result = false;
          colors = rtl.getObject(tooltip.labelColors[index]);
      ...
      tooltipEl.style["left"] = pas.SysUtils.IntToStr(positionX + tooltip.caretX) + "px";
      tooltipEl.style["top"] = pas.SysUtils.IntToStr(positionY + tooltip.caretY) + "px";
      tooltipEl.style["fontFamily"] = tooltip._bodyFontFamily;
      tooltipEl.style["fontSize"] = tooltip.bodyFontSize;
      tooltipEl.style["fontStyle"] = tooltip._bodyFontStyle;
      tooltipEl.style["padding"] = pas.SysUtils.IntToStr(tooltip.yPadding) + "px " + pas.SysUtils.IntToStr(tooltip.xPadding) + "px";
    };
    $mod.config = new Object();
    $mod.config.type = "pie";
    $mod.config.data = new Object();
    $mod.dataset = new Object();
    $mod.dataset.data = [300,50,100,40,10];
    $mod.dataset.backgroundColor = ["rgb(255, 99, 132)","rgb(255, 159, 64)","rgb(255, 205, 86)","rgb(75, 192, 192)","rgb(54, 162, 235)"];
    $mod.config.data.datasets = [$mod.dataset];
    $mod.config.data.labels = ["Red","Orange","Yellow","Green","Blue"];
    $mod.config.options = new Object();
    $mod.config.options.responsive = true;
    $mod.config.options.legend = new Object();
    $mod.config.options.legend.display = false;
    $mod.config.options.tooltips = new Object();
    $mod.config.options.tooltips.enabled = false;
    new Chart("chart-area",$mod.config);
  };
});
//# sourceMappingURL=demopie.js.map
