...
rtl.module("program",["System","JS","Web","SysUtils","Math","libjquery","ChartJS"],function () {
  "use strict";
  var $mod = this;
  ...
  $mod.$main = function () {
    $mod.customTooltips = function (tooltip) {
      var canvas = null;
      canvas = rtl.getObject(this["_chart"]).canvas;
      jQuery(canvas).css("cursor","pointer");
      jQuery(".chartjs-tooltip").css("opacity",0);
      if ((!(tooltip != null) || !(rtl.length(tooltip.dataPoints) > 0)) && (tooltip.opacity === 0)) return;
      if (tooltip.dataPoints.length > 0) tooltip.dataPoints.forEach(function (element, index, arr) {
        var Result = false;
        var tooltp = null;
        var dataPoint = null;
        var content = "";
        dataPoint = rtl.getObject(element);
        content = (new Array(dataPoint.xLabel,dataPoint.yLabel)).join(": ");
        tooltp = jQuery("#tooltip-" + pas.SysUtils.IntToStr(dataPoint.datasetIndex));
        tooltp.html(content);
        tooltp.css("opacity",1);
        tooltp.css("top",pas.SysUtils.FloatToStr(canvas.offsetTop + dataPoint.y) + "px");
        tooltp.css("left",pas.SysUtils.FloatToStr(canvas.offsetLeft + dataPoint.x) + "px");
        return Result;
      });
    };
    $mod.lineChartData = new Object();
    $mod.lineChartData.datasets = new Array();
    $mod.lineChartData.labels = ["January","February","March","April","May","June","July"];
    $mod.dataset = new Object();
    $mod.dataset.label = "My First dataset";
    $mod.dataset.backgroundColor = "rgba(255, 99, 132, 0.2)";
    $mod.dataset.borderColor = "rgb(255, 99, 132)";
    $mod.dataset.pointBackgroundColor = "rgb(255, 99, 132)";
    ...
    $mod.lineChartData.datasets.push($mod.dataset);
    $mod.dataset = new Object();
    $mod.dataset.label = "My Second dataset";
    $mod.dataset.backgroundColor = "rgba(54, 162, 235, 0.2)";
    $mod.dataset.borderColor = "rgb(54, 162, 235)";
    $mod.dataset.pointBackgroundColor = "rgb(54, 162, 235)";
    $mod.dataset.data = [$mod.randomScalingFactor(),$mod.randomScalingFactor(),$mod.randomScalingFactor(),$mod.randomScalingFactor(),$mod.randomScalingFactor(),$mod.randomScalingFactor(),$mod.randomScalingFactor()];
    $mod.lineChartData.datasets.push($mod.dataset);
    $mod.config = new Object();
    $mod.config.type = "line";
    $mod.config.data = $mod.lineChartData;
    $mod.config.options = new Object();
    $mod.config.options.title = new Object();
    $mod.config.options.title.display = false;
    $mod.config.options.title.text = "Chart.js - Custom Tooltips using Data Points";
    $mod.config.options.tooltips = new Object();
    $mod.config.options.tooltips.enabled = false;
    $mod.config.options.tooltips.mode = "index";
    $mod.config.options.tooltips.intersect = false;
    $mod.config.options.tooltips.custom = $mod.customTooltips;
    new Chart("chart1",$mod.config);
  };
});
//# sourceMappingURL=democustompoints.js.map
