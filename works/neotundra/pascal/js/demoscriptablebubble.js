...
rtl.module("ChartJS",["System","JS","Web"],function () {
  "use strict";
  var $mod = this;
  rtl.createClassExt($mod,"TChartXYVData",Object,"",function () {
    this.$init = function () {
      this.x = undefined;
      this.y = undefined;
      this.v = undefined;
...
rtl.module("program",["System","JS","Web","SysUtils","Math","ChartJS"],function () {
  ...
  this.rand = function (min, max) {
    var Result = 0;
    Result = pas.Math.RandomRange(min,max);
    return Result;
  };
  this.ftos = function (f) {
    var Result = "";
    Result = pas.SysUtils.FloatToStrF(f,3,15,1);
    return Result;
  };
  this.DATA_COUNT = 16;
  this.MIN_XY = -150;
  ...
  this.generateData = function () {
    ...
    Result = new Array();
    for (i = 0; i <= 15; i++) {
      Result.push(pas.ChartJS.TChartXYVData.$create("new$1",[$mod.rand(-150,100),$mod.rand(-150,100),$mod.rand(0,1000)]));
    };
    return Result;
  };
  this.colorize = function (opaque, context) {
    ...
    var b = 0.0;
    var a = 0.0;
    value = rtl.getObject(context.dataset.data[context.dataIndex]);
    x = value.x / 100;
    y = value.y / 100;
    r = pas.Math.IfThen((x < 0) && (y < 0),250,pas.Math.IfThen(x < 0,150,pas.Math.IfThen(y < 0,50,0)));
    g = pas.Math.IfThen((x < 0) && (y < 0),pas.Math.IfThen(x < 0,50,pas.Math.IfThen(y < 0,150,250)),0);
    b = pas.Math.IfThen((x < 0) && (y < 0),0,pas.Math.IfThen((x > 0) && (y > 0),250,150));
    a = pas.Math.IfThen$1(opaque,1,(0.5 * value.v) / 1000);
    Result = "rgba(" + $mod.ftos(r) + "," + $mod.ftos(g) + "," + $mod.ftos(b) + "," + $mod.ftos(a) + ")";
    return Result;
  };
  this.color = function (index) {
    var Result = "";
    Result = $mod.COLORS[index % 9];
    return Result;
  };
  this.chart = null;
  this.config = null;
  ...
  $mod.$main = function () {
    $mod.data = new Object();
    $mod.data.datasets = new Array();
    $mod.dataset = new Object();
    $mod.dataset.data = $mod.generateData();
    $mod.data.datasets.push($mod.dataset);
    $mod.dataset = new Object();
    $mod.dataset.data = $mod.generateData();
    $mod.data.datasets.push($mod.dataset);
    $mod.options = new Object();
    $mod.options.aspectRatio = 1;
    $mod.options.legend = false;
    $mod.options.tooltips = false;
    $mod.options.elements = new Object();
    $mod.options.elements.point = new Object();
    $mod.options.elements.point.backgroundColor = $mod.colorize.bind(null,false);
    $mod.options.elements.point.borderColor = $mod.colorize.bind(null,true);
    $mod.options.elements.point.borderWidth = function (context) {
      var Result = undefined;
      Result = Math.min(Math.max(1,context.datasetIndex + 1),8);
      return Result;
    };
    $mod.options.elements.point.hoverBackgroundColor = "transparent";
    $mod.options.elements.point.hoverBorderColor = function (context) {
      var Result = undefined;
      Result = $mod.color(context.datasetIndex);
      return Result;
    };
    $mod.options.elements.point.hoverBorderWidth = function (context) {
      var Result = undefined;
      var value = null;
      value = rtl.getObject(context.dataset.data[context.dataIndex]);
      Result = Math.round((8 * value.v) / 1000);
      return Result;
    };
    $mod.options.elements.point.radius = function (context) {
      var Result = undefined;
      var value = null;
      var size = 0;
      var base = 0.0;
    ...
    $mod.config.type = "bubble";
    $mod.config.data = $mod.data;
    $mod.config.options = $mod.options;
    $mod.chart = new Chart("chart-0",$mod.config);
    document.getElementById("randomize").addEventListener("click",function () {
      $mod.chart.config.data.datasets.forEach(function (element, index, arr) {
        var Result = false;
        rtl.getObject(element).data = $mod.generateData();
        return Result;
      });
      $mod.chart.update();
    });
    document.getElementById("addDataset").addEventListener("click",function () {
      var dataset = null;
      dataset = new Object();
      dataset.data = $mod.generateData();
      $mod.chart.config.data.datasets.push(dataset);
      $mod.chart.update();
    });
    document.getElementById("removeDataset").addEventListener("click",function () {
      $mod.chart.config.data.datasets.shift();
      $mod.chart.update();
    });
  };
});
//# sourceMappingURL=demoscriptablebubble.js.map
